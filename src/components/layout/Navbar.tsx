...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ["home", "projects", "about", "services", "forwho", "process", "testimonials", "faq", "contact"];
      ...
      for (const section of sections) {
        ...
        if (element) {
          ...
          if (elementTop <= scrollPosition) {
            currentSection = section === "forwho" ? "services" : section === "testimonials" || section === "faq" ? "contact" : section;
          }
        }
      }
  ...
  const navLinks = [
    { id: "projects", label: t("nav.projects"), href: "/#projects", isDropdown: true },
  ...
  const isProjectDetailPage = location.pathname.startsWith('/realizacje/') && location.pathname !== '/realizacje' && location.pathname !== '/realizacje/';
  const isDarkHeroContext = isProjectDetailPage && !isScrolled;
  const navTextColor = isDarkHeroContext ? "text-white" : "text-foreground/90 dark:text-foreground/80";
  const navTextHoverColor = isDarkHeroContext ? "hover:text-[#14B8A6]" : "hover:text-primary";
  const logoTextColor = isDarkHeroContext ? "text-white/90" : "text-foreground/80";
  const iconColor = isDarkHeroContext ? "text-white" : "text-foreground/80";
  const iconHoverClass = isDarkHeroContext ? "hover:text-[#14B8A6] hover:bg-white/10" : "hover:text-primary hover:bg-primary/10";

  return (
    <>
      <motion.header
        style={{
          backgroundColor: mobileMenuOpen ? "hsl(var(--background))" : (isDarkHeroContext ? "transparent" : navBackground),
          backdropFilter: mobileMenuOpen || isDarkHeroContext ? "none" : backdropBlur,
          borderBottomColor: mobileMenuOpen || isDarkHeroContext ? "transparent" : borderColor,
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8 absolute left-1/2 -translate-x-1/2">
            {navLinks.map((item) => (
              <div 
                ...
                onMouseEnter={() => item.isDropdown && setProjectsDropdownOpen(true)}
                onMouseLeave={() => item.isDropdown && setProjectsDropdownOpen(false)}
              >
                <HashLink 
                  smooth
                  to={item.href} 
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full bg-primary transition-opacity duration-300 ${activeSection === item.id && location.pathname === '/' ? "opacity-100 shadow-[0_0_8px_rgba(45,212,191,0.8)]" : "opacity-0 group-hover:opacity-50"}`} />
                  {item.label}
                  {item.isDropdown && <ChevronDown className="w-3 h-3 ml-0.5 opacity-70" />}
                </HashLink>
            ...
            <nav className="flex flex-col gap-6 mb-auto">
              {navLinks.map((item) => (
                <div key={item.id} className="flex flex-col gap-4">
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full bg-primary transition-opacity duration-300 ${activeSection === item.id && location.pathname === '/' ? "opacity-100 shadow-[0_0_8px_rgba(45,212,191,0.8)]" : "opacity-0"}`} />
                    {item.label}
                  </HashLink>
                  {item.isDropdown && (
            ...
            <div className="flex flex-col gap-6 mt-12">
              ...
              <div className="flex items-center justify-between border-t border-border pt-6">
                ...
                >
                  ...
                  <span>{i18n.language.toUpperCase() === "EN" ? "English" : "Polski"}</span>
                </button>
              </div>
